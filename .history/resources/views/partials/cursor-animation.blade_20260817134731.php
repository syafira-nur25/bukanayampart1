...
@media (pointer: fine) {
  ...
  /* Kepala ayam goyang-goyang lucu */
  #cursorChicken .head {
...
<!-- Elemen ayam (inline SVG biar tajam & ringan) -->
...
(function () {
  ...
  var aktif = false;
  var angle = 0; // rotasi ayam mengikuti arah gerak

  window.addEventListener('mousemove', function (e) {
    mx = e.clientX; my = e.clientY;
    if (!aktif) { aktif = true; rx = mx; ry = my; chicken.classList.add('on'); }

    // hitung arah gerak untuk rotasi ayam (dihaluskan)
    var dx = mx - lastX;
    var dy = my - lastY;
    if (Math.hypot(dx, dy) > 2) {
      // arahkan ke kanan = 0°, tapi ayam menghadap kanan, jadi cocok
      var target = Math.atan2(dy, dx) * 180 / Math.PI;
      // batasi rotasi biar tidak jungkir-balik (±25°)
  ...
  (function loop() {
    if (aktif) {
      ...
      // rotasi halus mengikuti arah + sedikit "angguk"
      chicken.style.transform = 'translate(-50%, -50%) rotate(' + angle.toFixed(1) + 'deg)';
    }
    requestAnimationFrame(loop);
  })();

  // ayam membesar saat di atas link/tombol
  ...
  // jejak telur saat mouse bergerak
  ...
  // klik = ayam "bertelur" → 8 telur pecah menyebar

...
<head>
    ...
    <style>
        ...
        .topbar { background: #2d1b16; }
        .topbar .navbar-brand i { display: inline-block; animation: wobble 2.6s ease-in-out infinite; }
...
<body>
    <nav class="topbar navbar navbar-dark px-3">
        <span class="navbar-brand fw-bold"><i class="bi bi-egg-fried me-2"></i>BukanAyam</span>
        <div class="d-flex align-items-center gap-2">
            <span class="text-white small d-none d-md-inline">{{ auth()->user()->name ?? '' }}</span>
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button class="btn btn-sm btn-outline-light"><i class="bi bi-box-arrow-right me-1"></i>Keluar</button>
            </form>
        </div>
    </nav>
